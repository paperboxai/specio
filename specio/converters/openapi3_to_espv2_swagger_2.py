...
class Converter:
    ...
    def __convert_infos(cls, spec: dict):
        ...
        if server:
            ...
            if url.scheme is not None and url.scheme != "":
                spec["schemes"] = [url.scheme[0 : len(url.scheme) - 1]]  # strip off trailing colon
            else:
            ...
            if param.get("type") == "array":
                ...
                if style == "matrix":
                    ...
                    param["collectionFormat"] = (
                        "csv" if not param.get("explode", False) else "multi"
                    )
        ...
        for code in deepcopy(operation)["responses"]:
            ...
            if response.get("content"):
                ...
                for media_range in response["content"]:
                    ...
                    media_type = (
                        media_range if "*" not in media_range else "application/octet-stream"
                    )
                    if not operation.get("produces"):
                        operation["produces"] = [media_type]
                    elif media_type not in operation["produces"]:
        ...
        if schema_type == "object":
            if schema.get("properties") is not None:
                for prop_name in schema["properties"]:
                    if (
                        schema["properties"][prop_name].get("writeOnly", False)
                        and operation_direction == "response"
                    ):
    ...
    def __convert_operation_parameters(cls, operation: dict):
        ...
        if operation.get("requestBody") is not None:
            ...
            if isinstance(content, dict) and len(content) > 0:
                ...
                if (
                    content_key == SUPPORTED_MIME_TYPES["APPLICATION_X_WWW_URLENCODED"]
                    or content_key == SUPPORTED_MIME_TYPES["MULTIPART_FORM_DATA"]
                ):
                    ...
                    if (
                        param["schema"].get("type") == "object"
                        and param["schema"].get("properties") is not None
                    ):
                    ...
                    operation["consumes"] = (
                        media_types if len(media_types) > 0 else ["application/octet-stream"]
                    )
                    param["in"] = "body"
                    param["name"] = param.get("name", "file")
                    param.pop("type", None)
                    param["schema"] = content[media_ranges[0]].get(
    ...
    def __get_supported_mimetypes(cls, content: dict):
        ...
        return list(filter(lambda x: x in MIME_VALUES or utils.is_json_mimetype(x), content.keys()))
